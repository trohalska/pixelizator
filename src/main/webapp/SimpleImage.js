...
SimpleImage = function () {
    ...
    let htmlImage = funMap[arguments.length].apply(this, arguments);
...
SimpleImage.prototype = {
    ...
    toArray: function () {
        ...
        let array = new Array();
        // nip 2012-7
        // 1. simple-way (this is as good or faster in letious browser tests)
        // let array = new Array(this.getWidth() * this.getHeight());
    ...
    __init: function (img) {
        try {
            ...
            if (img instanceof HTMLCanvasElement) {
                let canvasData = img.getContext('2d').getImageData(0, 0, this.width, this.height);
...
let __SimpleImageUtilities = (function () {
    // private globals
    // image needed to seed "sized" image
    let EMPTY_IMAGE_DATA = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAUAAAAFCAQAAAAnZu5uAAAAAXNSR0IArs4c6QAAABVJREFUeJxiYPgPhyQwAQAAAP//AwCgshjoJhZxhgAAAABJRU5ErkJggg==';
    ...
    return {
        ...
        makeHTMLImageFromURL: function (url, simpleImage) {
            let name = url.substr(0, url.indexOf(';'));
        ...
        makeHTMLImageFromSize: function (width, height) {
            ...
            let img = __SimpleImageUtilities.EMPTY_IMAGE.cloneNode(true);
            img.width = width;
            img.height = height;
            return img;
        },
        ...
        changeSize: function (canvasOld, newWidth, newHeight) {
            let canvasNew = __SimpleImageUtilities.makeHTMLCanvas('setSize_');
        ...
        funCheck: function (funcName, expectedLen, actualLen) {
            if (expectedLen != actualLen) {
                let s1 = (actualLen == 1) ? '' : 's';  // pluralize correctly
                let s2 = (expectedLen == 1) ? '' : 's';
                let message = 'You tried to call ' + funcName + ' with ' + actualLen + ' value' + s1 +
        ...
        rangeCheck: function (value, low, high, funName, coordName, size) {
            if (value < low || value >= high) {
                let message = 'You tried to call ' + funName + ' for a pixel with ' + coordName + '-coordinate of ' + value +
